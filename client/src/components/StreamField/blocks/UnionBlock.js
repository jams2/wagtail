...
export class UnionBlock {
  constructor(blockDef, placeholder, prefix, initialState, initialError) {
    ...
    dom.className = h(this.blockDef.meta.classname || '');
    placeholder.replaceWith(dom);

    if (this.blockDef.meta.helpText) {
      const helpWrapper = document.createElement('div');
      helpWrapper.className = 'c-sf-help';
      const helpInner = document.createElement('div');
      helpInner.className = 'help';
      // help text is left unescaped as per Django conventions
      helpInner.innerText = this.blockDef.meta.helpText;
      dom.appendChild(helpWrapper);
    }

    this.blockDef.childBlockDefs.forEach((childBlockDef) => {
      const childDom = document.createElement('div');
      childDom.dataset.contentpath = childBlockDef.name;
      const label = document.createElement('label');
      label.className = 'w-field__label';
      label.innerText = h(childBlockDef.meta.label);

      if (childBlockDef.meta.required) {
      ...
      blockWrapper.dataset.streamfieldBlock = '';
      childDom.appendChild(label);
      childDom.appendChild(blockWrapper);
      dom.appendChild(childDom);

      const childBlockElement = childDom.querySelector(
        '[data-streamfield-block]',
      );
      const labelElement = childDom.querySelector('label');
    ...
    this.container = dom;
    this.childBlocks.__union_type__.element.addEventListener('change', () =>
  ...
  updateChildVisibility() {
    const { __union_type__: activeType } = this.getState();
    for (const name in this.childBlocks) {
      if (name === '__union_type__') continue;
      if (name !== activeType) {
        this.childBlocks[name].element.parentElement.classList.add('w-hidden');
      } else {
        this.childBlocks[name].element.parentElement.classList.remove(
          'w-hidden',
        );
      }
    }
...
export class UnionBlockDefinition {
